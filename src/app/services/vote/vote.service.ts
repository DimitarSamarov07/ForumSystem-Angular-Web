...
export class VoteService {
  ...
  async registerVote(postId, userId, polarity: boolean) {
    ...
    if (voteObjInDB.length > 0) {
      return await this.changeVote(voteObjInDB[0], polarity)
    }
    ...
    const user = await this.userStore.findById<IUser>(userId);
  ...
  async changeVote(voteObj, newPolarity) {
    voteObj.polarity = newPolarity;
    await this.voteStore.save(voteObj);

    const user = await this.userStore.findById<IUser>(voteObj.user.objectId);
